...
pub fn check_req_create_user_dto(req_create_user_dto: &ReqCreateUserDto) -> Result<(), ValidateReqCreateUserDtoError> {
    if req_create_user_dto.username.is_empty() {
        return Err(ValidateReqCreateUserDtoError::EmptyString("username field must not be empty".to_string()));
    }
    if req_create_user_dto.first_name.is_empty() {
        return Err(ValidateReqCreateUserDtoError::EmptyString("first name field must not be empty".to_string()));
    }
    if req_create_user_dto.last_name.is_empty() {
        return Err(ValidateReqCreateUserDtoError::EmptyString("last name field must not be empty".to_string()));
    }
    if req_create_user_dto.email.is_empty() {
        return Err(ValidateReqCreateUserDtoError::EmptyString("email field must not be empty".to_string()));
    }
    if req_create_user_dto.password.is_empty() {
        return Err(ValidateReqCreateUserDtoError::EmptyString("password field must not be empty".to_string()));
    }
    if req_create_user_dto.username.len() < 3 || req_create_user_dto.username.len() > 15 {
        return Err(ValidateReqCreateUserDtoError::LengthError("username must be between 3 and 15 characters".to_string()));
    }
    if req_create_user_dto.first_name.len() < 3 || req_create_user_dto.first_name.len() > 20 {
        return Err(ValidateReqCreateUserDtoError::LengthError("first name must be between 3 and 20 characters".to_string()));
    }
    if req_create_user_dto.last_name.len() < 3 || req_create_user_dto.last_name.len() > 20 {
        return Err(ValidateReqCreateUserDtoError::LengthError("last name must be between 3 and 20 characters".to_string()));
    }
    if req_create_user_dto.password.len() < 6 || req_create_user_dto.password.len() > 15 {
        return Err(ValidateReqCreateUserDtoError::LengthError("password must be at least 6 characters to 15 characters".to_string()));
    }
